...
ajax("/getInfo", function (data) {
    ...
    if (data) {
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            str += '<span class="w100" url="" className="company2" curId="' + cur["id"] + '">';//->把当前用户的ID存储到自定义属性上,以后点击修改或者删除的时候我们只需要找到这个ID就知道操作的是那一条信息了
...
boxList.onclick = function (e) {
    ...
    if (tar.getAttribute("btnType") === "delete") {
        ...
        var class = tar.parentNode.getAttribute("company");
        var flag = window.confirm("您确定要删除编号为 [ " + customerID + " ] 的这一条信息吗?");
        if (flag) {
            //->确定要删除了
            var parameter = "id=" + customerID;
            parameter = encodeURIComponent(parameter);
            ajax(getURL(urlJSON[customerID]), function (data) {
                if (data && data["code"] == 0) {
...
// <a onclick=""
function del(url,data,id) {
    var manyt={}
}
var urlJSON = {"company":{"parse": "user/xxx", "url": "192.168.0.1", "data": "{id=}", "type": "get"},
                "courst":{ "parse": "v3/xxx", "url": "155.29.229.228", "data": "{}", "type": "post" },
"vv":{"parse": "v3/xxx", "url": "155.29.229.228", "data": "{}", "type": "post"}};
var jsonData = null;
function getURL(parse) {
    $.each(urlJSON, function (index, item) {
        if (item.parse == parse) {
...
function getURL(v) { courst
    var v= new Array('company':{"parse": "user/xxx", "url": "192.168.0.1", "data": "{id=}", "type": "get"},'name'='eeee')
    var urlJSON = [ {"parse": "user/xxx", "url": "192.168.0.1", "data": "{id=}", "type": "get"},
        { "parse": "v3/xxx", "url": "155.29.229.228", "data": "{}", "type": "post" }];
    var urlJSON1 = [ {"parse": "user/xxx", "url": "192.168.0.1", "data": "{id=}", "type": "get"},
        { "parse": "v3/xxx", "url": "155.29.229.228", "data": "{}", "type": "post" }];
    var urlJSON1 = [ {"parse": "user/xxx", "url": "192.168.0.1", "data": "{id=}", "type": "get"},
        { "parse": "v3/xxx", "url": "155.29.229.228", "data": "{}", "type": "post" }];
    return y;
